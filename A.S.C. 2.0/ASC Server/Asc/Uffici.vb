...
Public Class Uffici
    ...
    Dim x As Integer
    Dim FaiRipartireTimers As Integer = 0
    ...
    Private Sub Uffici_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        Try
            ...
            If My.Settings.NumeroUfficioUltimo <> 0 Then
                My.Settings.NumeroUfficioUltimo = 0
            End If
            If FaiRipartireTimers = 3 Then
                Main.Label8.Text = "LISTA UFFICI"
                Main.Timer7.Start()
            ElseIf FaiRipartireTimers = 2 Then
                Main.Label8.Text = "LISTA UFFICI"
                Main.Timer6.Start()
            ElseIf FaiRipartireTimers = 1 Then
                Main.Label8.Text = "LISTA UFFICI"
                Main.Timer6.Start()
                Main.Timer7.Start()
            End If
    ...
    Private Sub Uffici_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            If Main.Timer6.Enabled = True And Main.Timer7.Enabled = True Then
                Main.Label8.Text = "LISTA UFFICI [Impost. Uffici Aperto]"
                FaiRipartireTimers = 1
            Else
                If Main.Timer6.Enabled = True Then
                    Main.Label8.Text = "LISTA UFFICI [Impost. Uffici Aperto]"
                    FaiRipartireTimers = 2
                    Main.Timer6.Enabled = False
                End If
                If Main.Timer7.Enabled = True Then
                    Main.Label8.Text = "LISTA UFFICI [Impost. Uffici Aperto]"
                    FaiRipartireTimers = 3
                    Main.Timer7.Enabled = False
                End If
            End If
        Catch ex As Exception
    ...
    Public Sub Rimozione()
        ...
        Try
            ...
            For Each oldfilename As String In My.Computer.FileSystem.GetFiles(Application.StartupPath & "\Uffici")
                My.Computer.FileSystem.RenameFile(oldfilename, "ufficiotemporaneo" & change & ".txt")
                change += 1
            Next
        ...
        Try
            If FolderBrowserDialog1.ShowDialog = Windows.Forms.DialogResult.OK Then
                ...
                If DirectoryName.Name.ToString = "Uffici" Then
                    ...
                    MsgBox("Importazione eseguita con successo.", MsgBoxStyle.Information, "Asc Server - Importa Uffici")
                    Timer1.Start()
                Else
                    MsgBox("Per poter importare gli uffici bisogna selezionare la cartella" & """" & "Uffici" & """" & ", contenente la lista degli uffici.", MsgBoxStyle.Information, "Asc Server - Importa Uffici")
                    Exit Sub
                End If
            End If
        Catch ex As Exception
            MsgBox("Errore durante l'importazione, ex message: " & ex.Message, MsgBoxStyle.Information, "Asc Server - Importa Uffici")
        End Try
