...
Public Class DATABASE_PARTENZE
    ...
    Public Sub UseMessage()
        ...
        Try
            For Each cell In DataGridView1.SelectedCells
                ...
                leggi = System.IO.File.OpenText(Application.StartupPath & "\Email\DATABASEPARTENZE\" & datadefinitiva(0) & "\emailinviate" & (cell.RowIndex + 1).ToString("D4") & "P.txt")
                letto = leggi.ReadToEnd
                leggi.Close()
                Main.TextBox3.Text = letto
                Main.TopMost = True
                Main.TopMost = False
                Main.TextBox3.SelectionStart = 0
                Main.TextBox3.SelectionLength = 0
                Me.Close()
            Next
    ...
    Private Sub DATABASE_PARTENZE_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing

        My.Settings.DTBPartenzeActive = 0
        My.Settings.SelectedDate2 = ""

    End Sub

    Private Sub DATABASE_PARTENZE_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        Try
            datagiorno = My.Settings.DataOggi
            My.Settings.DTBPartenzeActive = 1
    ...
    Public Sub Load22TerList()
        ...
        If My.Settings.SelectedDate2 <> "" Then
            Try
                If System.IO.File.Exists(Application.StartupPath & "\Email\DATABASEPARTENZE\" & My.Settings.SelectedDate2 & "\datiemailinviate" & My.Settings.SelectedDate2 & ".txt") Then
                    leggi = System.IO.File.OpenText(Application.StartupPath & "\Email\DATABASEPARTENZE\" & My.Settings.SelectedDate2 & "\datiemailinviate" & My.Settings.SelectedDate2 & ".txt")
                    letto = leggi.ReadToEnd
                    leggi.Close()
                    info = Split(letto, "#")
                    ...
                    LoadNr()
                ElseIf letto = "" Then
                    MsgBox("Non sono state inviate, nella data selezionata, email da visualizzare!", MsgBoxStyle.Information, "Asc Server - Database Partenze")
                End If
            Catch ex As Exception
    ...
    Public Sub LoadList()
        ...
        If My.Settings.SelectedDate2 = "" Then
            If My.Settings.SelectedDate = "" Then
                ...
                Try
                    If System.IO.File.Exists(Application.StartupPath & "\Email\DATABASEPARTENZE\" & datagiorno & "\datiemailinviate" & datagiorno & ".txt") Then
                        leggi = System.IO.File.OpenText(Application.StartupPath & "\Email\DATABASEPARTENZE\" & datagiorno & "\datiemailinviate" & datagiorno & ".txt")
                        letto = leggi.ReadToEnd
                        leggi.Close()
                        info = Split(letto, "#")
                        contMail = Split(letto, vbCrLf)
                        ...
                        LoadNr()
                    ElseIf letto = "" Then
                        MsgBox("Non sono state inviate, nella data selezionata, email da visualizzare!", MsgBoxStyle.Information, "Asc Server - Nessuna Email Presente")
                    Else
                        MsgBox("Non sono state inviate, nella data selezionata, email da visualizzare!", MsgBoxStyle.Information, "Asc Server - Nessuna Email Presente")
                    End If
                Catch ex As Exception
            ...
            Else
                Try
                    If System.IO.File.Exists(Application.StartupPath & "\Email\DATABASEPARTENZE\" & My.Settings.SelectedDate & "\datiemailinviate" & My.Settings.SelectedDate & ".txt") Then
                        leggi = System.IO.File.OpenText(Application.StartupPath & "\Email\DATABASEPARTENZE\" & My.Settings.SelectedDate & "\datiemailinviate" & My.Settings.SelectedDate & ".txt")
                        letto = leggi.ReadToEnd
                        leggi.Close()
                        info = Split(letto, "#")
                        contMail = Split(letto, vbCrLf)
    ...
    Public Sub Print22TerList()
        ...
        Try
            TerPrintPageNum = My.Settings.TerPrintPageNum + 1
    ...
    Public Sub cercaemail()
        ...
        Try
            For Each cell In DataGridView1.SelectedCells
                ...
                My.Settings.OpenedFilePath = Application.StartupPath & "\Email\DATABASEPARTENZE\" & datadefinitiva(0) & "\emailinviate" & (cell.RowIndex + 1).ToString("D4") & "P.txt"
                MessageText.Show()
            Next
        Catch ex As Exception
    ...
    Public Sub salvaemail()
        ...
        If DataGridView1.SelectedCells.Count > 1 Then
            MsgBox("Devi selezionare un'unica casella, quella contenete il numero progressivo dell'email che vuoi salvare.", MsgBoxStyle.Information, "Asc Server - Errore Selezione Casella")
        Else
            Dim cell As DataGridViewCell
            Try
                For Each cell In DataGridView1.SelectedCells
    ...
    Public Sub DeleteSelectedEmail()
        ...
        Try
            ...
            If DataGridView1.SelectedCells.Count = 1 Then
                Dim AlertMessage = MsgBox("Il messaggio selezionato verrà eliminato definitivamente, si è sicuri di voler procedere?", vbYesNo, "Asc Server - Eliminazione Messaggio")
                If AlertMessage = 6 Then

                    System.IO.File.Delete(Application.StartupPath & "\Email\DATABASEPARTENZE\" & datagiorno & "\datiemailinviate" & datagiorno & ".txt")

                    Dim cell As DataGridViewCell
                    For Each cell In DataGridView1.SelectedCells
                        If cell.Value.ToString <> "" Then
                            System.IO.File.Delete(Application.StartupPath & "\Email\DATABASEPARTENZE\" & datagiorno & "\emailinviate" & (cell.RowIndex + 1).ToString("D4") & "A.txt")
                            DataGridView1.Rows.RemoveAt(cell.RowIndex)
                        Else
                            MsgBox("La cella selezionata non contiene alcun messaggio, eliminazione annullata.", MsgBoxStyle.Information, "Asc Server - Eliminazione Annullata")
                            Exit Sub
                        End If
                    Next
                    ...
                    For r = 0 To DataGridView1.Rows.Count - 4
                        ...
                        stringadefinitiva += My.Settings.NumEmail2.ToString("D4") & "P - " & My.Settings.JlDate.ToString("D3") & "#"
                        ...
                        stringadefinitiva += DataGridView1.Rows(r).Cells(2).Value.ToString & "#" & DataGridView1.Rows(r).Cells(3).Value.ToString & "#" _
                            ...
                            & DataGridView1.Rows(r).Cells(6).Value.ToString & "#" & vbCrLf
                    Next
                    ...
                    Dim FilesCollection2() As String = System.IO.Directory.GetFiles(Application.StartupPath & "\Email\DATABASEPARTENZE\" & datagiorno)
                    For z = 1 To UBound(FilesCollection2)
                        My.Computer.FileSystem.RenameFile(FilesCollection2(z), "emailinviate" & z.ToString("D4") & "P.txt")
                    Next

                Else
                    MsgBox("Eliminazione del messaggio annullata.", MsgBoxStyle.Information, "Asc Server - Eliminazione Annullata")
                    Exit Sub
                End If
            Else
                MsgBox("Devi selezionare massimo e non più di una cella, eliminazione annullata.", MsgBoxStyle.Information, "Asc Server - Selezione Cella")
                Exit Sub
            End If
            MsgBox("Messaggio eliminato con successo.", MsgBoxStyle.Information, "Asc Server - Eliminazione Messaggio")
        Catch ex As Exception
            MsgBox("Errore durante l'eliminazione del messaggio: " & ex.Message, MsgBoxStyle.Critical, "Asc Server - Errore")
        End Try
